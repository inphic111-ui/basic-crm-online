...
### 修正顯示問題（第三十一階段）
- [x] 實現說話者分離 - 使用 GPT 分析轉錄文本，添加「業務：」和「客戶：」標籤
- [x] 簡繁轉換 - 將轉錄文本、分析總結、AI標籤從簡體字轉為繁體字
- [x] 修正業務欄位 - 保持空白（顯示 `-`）
- [x] 修正時間解析 - 按照檔名邏輯正確擷取日期和時間
...
### 溝通紀錄時間軸
- [x] 修改 CSV 上傳邏輯，添加 communication_timeline 記錄
- [x] 修改音檔分析邏輯，添加 communication_timeline 記錄
- [x] 確保時間軸按時間倒序排列

### 消費者輪廓自動生成
- [x] 實現基於文本的初步輪廓生成
- [x] 創建 AI 客戶分析服務模組（ai-customer-analysis.mjs）
- [x] 整合 AI 分析到 CSV 上傳流程（異步執行）
- [x] 實現雷達圖 6 個維度分數生成（購買意願、預算能力、決策急迫性、信任程度、溝通品質、回購潛力）
- [x] 實現銷售分析和建議策略生成
- [x] 實現詳細分析報告生成（Markdown 格式）
- [x] 實現客戶資訊自動提取（公司、職稱、電話、郵件、地址）
- [x] 實現產品資訊自動提取（類別、規格）
- [x] 實現報價資訊自動提取（金額、日期、狀態）
- [x] 實現決策結構分析（決策者、影響者、使用者）
- [x] 添加資料庫遷移邏輯（自動添加 AI 分析欄位）
- [ ] 實現基於音檔的輪廓增強功能
- [ ] 提取報價數字並計算 V/P 評分
...
## 前端顯示問題修復（優先級 1）
- [ ] 修改 CustomerDetailModal.jsx 中的雷達圖數據源
- [ ] 從資料庫讀取 radar_purchase_intention 等 6 個欄位
- [ ] 顯示 AI 生成的成交機率
- [ ] 顯示 AI 生成的銷售分析（優勢、劣勢、關鍵顧慮、建議策略）
- [ ] 顯示溝通紀錄時間軸
- [ ] 修復「AI 洞察分析」和「關鍵需求識別」的硬編碼文本
